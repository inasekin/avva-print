...
?>
    <div class="address-edit__title">Изменить адрес</div>
    <div class="address-edit__back-link">
        <a href="<?=$arParams["PATH_TO_LIST"]?>"><span><?=GetMessage("SPPD_RECORDS_LIST")?></span></a>
    </div>
...
if($arResult["ID"] <> '')
{
    ...
    <div class="address-edit__content">
        <form method="post" class="cabinet__form form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
            ...
            <div class="form__row">
                ...
                <div class="form__column">
                    <div class="form__input">
                        <label style="margin-bottom: 30px; display: block;">
                            <p><?= Loc::getMessage('SPPD_PROFILE_NO', array("#ID#" => $arResult["ID"]))?></p>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="form__row">
                <div class="form__column">
                    <div class="form__input">
                        <label>
                            <p><?=Loc::getMessage('SALE_PERS_TYPE')?></p>
                            <div class="immutable-text"><?=$arResult["PERSON_TYPE"]["NAME"]?></div>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="form__row">
                <div class="form__column">
                    <div class="form__input">
                        <label>
                            <p><?=Loc::getMessage('SALE_PNAME')?></p>
                            <input class="form-control" type="text" name="NAME" maxlength="50" id="sale-personal-profile-detail-name" value="<?=$arResult["NAME"]?>" placeholder="Название профиля доставки для удобного выбора при заказе "/>
                        </label>
                    </div>
                </div>
            </div>
            ...
            foreach($arResult["ORDER_PROPS"] as $block)
            {
                if (!empty($block["PROPS"]))
                {
                    ...
                    <div class="row">
                        <div class="col">
                            <h4 class="title-address"><?= $block["NAME"]?></h4>
                        </div>
                    </div>
                    <?
                    foreach($block["PROPS"] as $property)
                    {
                        ...
                        if ($property["TYPE"] == "CHECKBOX")
                        {
                            ...
                            <div class="form-check row">
                                <label class="col-form-label <?=$alignTop?>" for="sppd-property-<?=$key?>">
                                    ...
                                    {
                                        ?><span class="req">(обязательно)</span><?
                                    }
                                    ?>
                                </label>
                                <div>
                        ...
                        {
                            ?>
                            <div class="form-group row property-<?=$key?>">
                                <label class="col-form-label <?=$alignTop?>" for="sppd-property-<?=$key?>">
                                    <?= $property["NAME"]?>:
                                    <? if ($property["REQUIED"] == "Y")
                                    {
                                ...
                                <div>
                                    ...
                                    {
                                        if($key == 11 && $currentValue == ''){
                                            $currentValue = 'г. Москва';
                                        }
                                        ?>
                                        <input class="form-control" type="text" name="<?=$name?>" maxlength="50" id="sppd-property-<?=$key?>" value="<?=$currentValue?>" <?if($property["REQUIED"] == "Y"):?>required<?endif;?>/>
                                        <?
                                    }
                                    ?>
                                </div>
                        ...
                        elseif ($property["TYPE"] == "SELECT")
                        {
                            ...
                            <div class="form-group row">
                                ...
                                <div>
                                    <select class="form-control" name="<?=$name?>" id="sppd-property-<?=$key?>" size="<?echo (intval($property["SIZE1"])>0)?$property["SIZE1"]:1; ?>">
                                        <?
                        ...
                        elseif ($property["TYPE"] == "MULTISELECT")
                        {
                            ...
                            <div class="form-group row">
                                ...
                                <div>
                                    <select class="form-control" id="sppd-property-<?=$key?>" multiple name="<?=$name?>[]" size="<?echo (intval($property["SIZE1"])>0)?$property["SIZE1"]:5; ?>">
                                        <?
                                        $arCurVal = array();
                                    ...
                                    foreach ($currentValue as $dataInputValue)
                                    {
                                        ...
                                        <div class="form-group">
                                            <div class='input-group date'>
                                                <input class="form-control" type="text" name="<?=$name?>" maxlength="50" value="<?=$dataInputValue?>"/>
            ...
            ?>
            <div class="profile__detail_btns">
                <div class="profile__detail_btns-inner">
                    <input type="submit" class="profile__detail_btn btn__save" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
                    <input type="submit" class="profile__detail_btn btn__apply"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">
                    <input type="submit" class="profile__detail_btn btn__reset"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
                </div>
            </div>
        </form>
    </div>
